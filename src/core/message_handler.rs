...
pub fn handle_message(sutom_msg: &String) -> Result<Party, String> {
    let taille_mot_opt = get_taille_du_mot_from(sutom_msg);
    let nombre_essai_total_opt = get_nombre_essaies_total(sutom_msg);
    let nombre_essai_opt = get_nombre_essaies(sutom_msg);
